...
export default function Flights() {
  ...
  useEffect(() => {
    ...
    // Injecter un style tag avec !important pour overrider TP
    const styleTag = document.createElement('style');
    styleTag.id = 'hf-tp-override';
    styleTag.textContent = `
      ...
      [class*="tpwl-search-header"][style*="position: sticky"],
      [class*="tpwl-search-header"][style*="position:sticky"] {
    ...
    `;
    if (!document.getElementById('hf-tp-override')) {
      document.head.appendChild(styleTag);
    }

    // Observer JS en backup
    const observer = new MutationObserver(() => {
      document.querySelectorAll('[class*="tpwl__content"]').forEach(el => {
        if (el.style.maxWidth !== '1280px') {
      ...
      document.querySelectorAll('[class*="tpwl-search__wrapper"],[class*="tpwl-tickets__wrapper"]').forEach(el => {
